...
@Composable
fun ContactSharingPage(
    navController: NavController,
    viewModel: ContactShareViewModel,
) {
    ...
    Scaffold(
        ...
        Column(
            ...
            when (val s = state) {
                ContactShareViewModel.UiState.Loading ->
                    Box(
                        modifier = Modifier
                            .fillMaxWidth()
                            .heightIn(min = 220.dp),
                ...
                ContactShareViewModel.UiState.Offline ->
                    ErrorState(
                ...
                is ContactShareViewModel.UiState.Error ->
                    UiErrorState(error = s.error, icon = null, onRetry = viewModel::load)

                is ContactShareViewModel.UiState.Loaded -> {
                    EntityHeaderRow(
                        icon = Icons.Filled.Person,
                        title = s.contact.fullName,
                        subtitle = stringResource(R.string.entity_kind_contact),
                    )
